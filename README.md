...
The build folder is included in the repository for easy deployment. If you want to build yourself, the detailed instructions are given [below](#usage).
 ...
 3. `npx truffle migrate --network ropsten --reset --f 2 --to 3`
 4. `npx truffle migrate --network ropsten --reset --f 5`

For each additional denomination that you want to deploy a contract, repeat step 2 and 4. i.e. change TOKEN_AMOUNT and then run  `npx truffle migrate --network ropsten --reset --f 5` again.

5. Copy the address generated by step 4, to use as configuration for the UI at [tBTC+TornadoCash](https://github.com/zyra-zia/tBTC-TornadoCash)
